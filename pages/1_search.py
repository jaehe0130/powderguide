...
# ----------------------------------------
# Ski Resort Data (name / region / url / image / lat lon)
# ----------------------------------------
ski_resorts = [
    {"name": "휘닉스 평창", "region": "강원도", "lat": 37.5795, "lon": 128.3257,
     "url": "https://phoenixhnr.co.kr", "image": "https://phoenixhnr.co.kr/images/sub/ski.jpg"},

    {"name": "비발디파크", "region": "강원도", "lat": 37.6512, "lon": 127.6841,
    ...
    {"name": "오크밸리", "region": "강원도", "lat": 37.4488, "lon": 127.8238,
     "url": "https://www.oakvalley.co.kr", "image": "https://www.oakvalley.co.kr/images/sub/ski/ski_01.jpg"},

    {"name": "웰리힐리파크", "region": "강원도", "lat": 37.4883, "lon": 128.2422,
...
def get_static_map(lat, lon):
    return f"https://maps.googleapis.com/maps/api/staticmap?center={lat},{lon}&zoom=13&size=400x300&maptype=roadmap&markers=color:red%7C{lat},{lon}&key={GOOGLE_MAPS_KEY}"
...
스키장 사진 + 위치 지도 + 날씨 정보 확인하기
</p>
""", unsafe_allow_html=True)

st.markdown("---")
st.subheader("🔍 스키장 이름 또는 지역 검색")
keyword = st.text_input("검색 (예: 평창 / 강원도 / 용평 / 경기)", "").strip()
...
# Display Results
# ----------------------------------------
st.markdown(f"### 검색 결과: {len(filtered_df)}곳")

if len(filtered_df) == 0:
    st.info("조건에 맞는 스키장이 없습니다 😥")
else:
    for resort in filtered_df.to_dict(orient="records"):
        st.markdown("---")
        ...
        with col2:
            ...
            if weather:
                ...
                st.write("날씨 정보 로드 실패")

            st.markdown(f"🔗 [공식 홈페이지 방문하기]({resort['url']})")
